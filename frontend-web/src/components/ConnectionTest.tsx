...
export function ConnectionTest() {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      // Test API health
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://clicktosell.onrender.com'}/api/health`);
        if (response.ok) {
    ...
    <div className="fixed bottom-4 right-4 bg-white border border-gray-200 rounded-lg shadow-lg p-4 w-80 z-50">
      ...
      <div className="space-y-3">
        ...
        <div className="flex space-x-2 mt-4">
          ...
          <button
            onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL || 'https://clicktosell.onrender.com'}/health`, '_blank')}
